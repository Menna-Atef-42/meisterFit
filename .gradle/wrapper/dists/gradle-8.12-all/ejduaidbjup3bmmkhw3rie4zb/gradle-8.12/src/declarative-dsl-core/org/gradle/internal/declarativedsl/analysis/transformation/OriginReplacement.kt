...
object OriginReplacement {
     ...
     */
    fun replaceReceivers(
        replaceIn: ObjectOrigin,
        predicate: (ObjectOrigin.ReceiverOrigin) -> Boolean,
        replacement: ObjectOrigin.ReceiverOrigin
    ): ObjectOrigin =
        ReceiverReplacementContext(predicate, replacement).replace(replaceIn)

    private
    class ReceiverReplacementContext(
        val predicate: (ObjectOrigin.ReceiverOrigin) -> Boolean,
        val replacement: ObjectOrigin.ReceiverOrigin
    ) {
        fun replace(origin: ObjectOrigin): ObjectOrigin =
            when (origin) {
                ...
                is ObjectOrigin.ImplicitThisReceiver -> origin.copy(
                    resolvedTo = replaceInReceiver(
                        origin.resolvedTo
                    )
                )
        ...
        fun replaceInReceiver(receiverOrigin: ObjectOrigin.ReceiverOrigin) =
            if (predicate(receiverOrigin)) replacement else when (receiverOrigin) {
                ...
                is ObjectOrigin.ConfiguringLambdaReceiver -> receiverOrigin.copy(
                    receiver = replace(
        ...
        private
        fun replaceInFunction(origin: ObjectOrigin.FunctionOrigin): ObjectOrigin.FunctionOrigin =
            when (origin) {
                is ObjectOrigin.AddAndConfigureReceiver -> replaceIn(origin)
                is ObjectOrigin.AccessAndConfigureReceiver -> replaceIn(origin)
                is ObjectOrigin.BuilderReturnedReceiver -> origin.copy(
                    receiver = replace(origin.receiver),
                    parameterBindings = replaceInArgs(origin.parameterBindings)
                )

                is ObjectOrigin.ConfiguringLambdaReceiver -> origin.copy(
                    receiver = replace(origin.receiver),
                    parameterBindings = replaceInArgs(origin.parameterBindings)
                )

                is ObjectOrigin.NewObjectFromMemberFunction -> origin.copy(
                    receiver = replace(origin.receiver),
                    parameterBindings = replaceInArgs(origin.parameterBindings)
                )

                is ObjectOrigin.NewObjectFromTopLevelFunction -> origin.copy(
                    parameterBindings = replaceInArgs(
