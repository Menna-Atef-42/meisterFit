...
class KotlinGrammar : Combinator(true, true) {
    ...
    val annotationUseSiteTarget by debug {
        (symbol("field") + symbol("property") + symbol("get") + symbol("set") +
                symbol("receiver") + symbol("param") + symbol("setparam") + symbol("delgate")) * token(
            COLON
        )
    }
    ...
    val functionTypeParameters by debug {
        paren(
            optional(parameter + type) * zeroOrMore(token(COMMA) * (parameter + type)) * optional(
                token(COMMA)
            )
        )
    }
    ...
    init {
        ...
        fileAnnotation =
            annotationMarker * symbol("file") * token(COLON) * (unescapedAnnotation + listOfUnescapedAnnotations)
        ...
        directlyAssignableExpression = debugReference {
            (postfixUnaryExpression * assignableSuffix) + simpleIdentifier + paren(
